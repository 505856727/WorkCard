 ...
 * 创建包并按规范补齐标准子目录
 */
function createScaffoldPackage(rawName: string): void {
...
function promptCreate(): void {
  ...
  App.Input("请输入新包名称", "", (name: string) => createScaffoldPackage(name));
}

/** 取文件夹 item 的内部路径，形如 "/Windows/"；根目录返回 "/" */
function folderInnerPath(folder: FairyEditor.FPackageItem): string {
  if (folder.isRoot) {
    return "/";
  }
  let p = folder.path || "/";
 ...
 */
function getTargetLocation(): { pkg: FairyEditor.FPackage; path: string } | null {
 ...
 */
function createWindowComponent(): void {
  if (!App.project || !App.project.opened) {
    App.Alert("请先打开一个工程");
    return;
  }
  ...
  App.Input("请输入窗口名称", "NewWindow", (raw: string) => {
    ...
    try {
      const pkg = loc.pkg;
      pkg.EnsureOpen();
      const pi = pkg.CreateComponentItem(
        name,
        WINDOW_WIDTH,
        WINDOW_HEIGHT,
        loc.path,
      ...
      console.log(
        `PackageScaffold: 已创建窗口 ${name} (${WINDOW_WIDTH}x${WINDOW_HEIGHT}) @ ${pkg.name}${loc.path}`,
      );
...
const libView = App.libView as any;
...
if (libView.contextMenu_Folder) {
  libView.contextMenu_Folder.AddItem(
    WINDOW_MENU_CAPTION,
    "pkgscaffold_new_window_folder",
    createWindowComponent,
  );
}
if (libView.contextMenu_Package) {
  libView.contextMenu_Package.AddItem(
    WINDOW_MENU_CAPTION,
    "pkgscaffold_new_window_pkg",
    createWindowComponent,
  );
}
